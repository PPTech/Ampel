...
ModelType = Literal["yolov8_nano", "efficientdet_lite"]
...
class EdgeDetector:
    ...
    def _load_backend(self, config: ModelConfig):
        ...
        # 1) YOLOv8-Nano: ultralytics.YOLO(config.weights_path)
        # 2) EfficientDet-Lite: tflite_runtime.Interpreter(model_path=config.weights_path)
        # This placeholder keeps the API stable for mobile conversion.
        if config.model_type not in ("yolov8_nano", "efficientdet_lite"):
