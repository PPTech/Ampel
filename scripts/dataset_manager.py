...
def sync_datasets() -> int:
    data = load_manifest()
    rows = list(data.get("datasets", [])) + load_manual_extra()
    con = init_db()
    cur = con.cursor()
    now = int(time.time())
    for ds in rows:
    ...
    con.commit()
    con.close()
    print(json.dumps({"synced": len(rows), "db": str(DB_PATH)}, ensure_ascii=False))
    return 0
...
def check_online() -> int:
    ...
    for ds in data.get("datasets", []):
        ...
        notes = ""
        try:
            req = Request(url, headers={"User-Agent": "traffic-ai-assist-dataset-manager/0.6.0"}, method="GET")
...
def build_training_plan(output: Path) -> int:
    ...
    plan = {
        "version": "0.6.0",
        "strategy": [
            "1) Download datasets with legal permission and keep original licenses",
            "2) Convert labels to unified COCO-like schema",
            "3) Split train/val/test with route/geography balancing",
            "4) Train detector baseline (FasterRCNN or RT-DETR)",
            "5) Distill to edge model for mobile/embedded inference",
            "6) Validate with AB and safety policy checks"
        ],
        "datasets": rows,
    }
