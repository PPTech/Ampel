...
class LearningAgent:
    ...
    def log_event(self, event_type: str, payload: Dict[str, object]) -> None:
        ...
        cur = self.db.conn.cursor()
        cur.execute("INSERT INTO audit_log(ts,event_type,payload) VALUES(?,?,?)", (int(time.time()), event_type, safe_json_dumps(safe)))
        self.db.conn.commit()
...
def core_ai_models() -> List[Dict[str, str]]:
    return [
        ...
        {"name": "EfficientDet-Lite0", "runtime": "TensorFlow Lite", "license": "Apache-2.0", "role": "Traffic light/lane detection core candidate"},
        {"name": "NVIDIA TAO VehicleTypeNet (optional)", "runtime": "ONNX/TensorRT", "license": "NGC model terms", "role": "Optional vehicle context enrichment (local artifact only)"},
    ]
        ...
        class Handler(BaseHTTPRequestHandler):
            ...
            def do_POST(self) -> None:  # noqa
                ...
                if self.path == "/ops/clear-data":
                    self._send(HTTPStatus.OK, {"ok": True, "action": "clear-data", "result": clear_all_local_data(db)})
                    return
                self._send(HTTPStatus.NOT_FOUND, {"error": "not found", "path": self.path})
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="Traffic AI Assist v0.9.2")
