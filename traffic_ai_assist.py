...
<style>body{font-family:Arial;background:#111;color:#eee;margin:0;padding:12px}video{width:100%;max-width:900px;background:#000;border:1px solid #333}pre{background:#0f1624;padding:8px}.hint{color:#9fb7ff}</style>
...
<p class='hint'>Single camera preview is shown. Object detection uses COCO-SSD in browser when network permits, with color-heuristic fallback.</p>
...
let model=null;
const hidden=document.createElement('canvas'); hidden.width=640; hidden.height=360;
const hctx=hidden.getContext('2d');

async function loadModel(){
  try{ model = await cocoSsd.load(); det.textContent='model loaded'; }
...
function fallbackDetect(){
  ...
  const img=hctx.getImageData(0,0,hidden.width,hidden.height).data;
  ...
  const objects=[];
  if(red>350) objects.push('traffic_light_red');
  if(green>350) objects.push('traffic_light_green');
  if(yellow>240) objects.push('traffic_light_yellow');
  if(bright>900) objects.push('vehicle_or_reflection');
...
async function detectObjects(){
  ...
  if(model){
    ...
    const labels=preds.map(p=>`${p.class}:${p.score.toFixed(2)}`);
    det.textContent=JSON.stringify({mode:'coco-ssd',objects:labels},null,2);
  } else {
    det.textContent=JSON.stringify({mode:'fallback',objects:fallbackDetect()},null,2);
  }
}
...
(async()=>{
  ...
  setInterval(()=>{detectObjects().catch(err=>det.textContent='detect error: '+err);},900);
})();
</script>
</body></html>"""
    return html.replace('__APP__', APP_NAME).replace('__VER__', SEMVER)
...
<div class='card'><h3>Demo constraints</h3><ul><li>Route locked to <b>berlin-city-center</b></li><li>Dataset switch changes sample source only</li><li>Health endpoint remains JSON for monitoring</li></ul></div>
</div></body></html>"""
