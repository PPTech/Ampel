...
async function loadModel(){
  try{ model = await cocoSsd.load(); det.textContent='model loaded: coco-ssd'; }
  catch(e){ det.textContent='model unavailable, fallback mode: '+e; model = null; }
}
...
async function detectObjects(){
  ...
  drawBoxes(preds);
  det.textContent=JSON.stringify({mode:model?'coco-ssd':'fallback', objects:preds.slice(0,10)},null,2);
}
(async()=>{
  ...
  function stopCam(){ if(activeStream){activeStream.getTracks().forEach(t=>t.stop()); activeStream=null;} video.srcObject=null; ctx.clearRect(0,0,canvas.width,canvas.height); det.textContent='camera stopped'; }
...
(async()=>{
  ...
  document.getElementById('runDemo').onclick=async()=>{
    ...
    warn.style.background=(d.event && d.event.alert && d.event.alert!=='ok') ? 'rgba(200,40,40,0.85)' : 'rgba(40,130,70,0.85)';
...
def core_ai_models() -> List[Dict[str, str]]:
    return [
        {"name": "COCO-SSD", "runtime": "Browser TensorFlow.js", "license": "Apache-2.0", "role": "Developer mode object detection"},
        {"name": "YOLOv8n", "runtime": "Edge/mobile conversion target", "license": "AGPL-3.0/commercial options", "role": "Traffic light/lane detection candidate"},
    ]
        ...
        class Handler(BaseHTTPRequestHandler):
            ...
            def do_POST(self) -> None:  # noqa
                ...
                if self.path == "/ops/sync-datasets":
                    ...
                    self._send(HTTPStatus.OK, {"ok": True, "action": "sync-datasets", "health": health_payload(db)})
                    return
                if self.path == "/ops/import-datasets":
                    proc = subprocess.run(["./scripts/import_all_datasets_local.sh"], capture_output=True, text=True)
                    self._send(HTTPStatus.OK, {"ok": proc.returncode == 0, "action": "import-datasets", "returncode": proc.returncode, "stdout": proc.stdout[-1200:], "stderr": proc.stderr[-1200:]})
                    return
                if self.path == "/ops/train-agent":
                    params = self._json_body()
                    epochs = int(params.get("epochs", 3))
                    DatasetBootstrapper(db, logger).seed_demo_samples()
                    self._send(HTTPStatus.OK, {"ok": True, "action": "train-agent", "result": train_agent_model(db, epochs=epochs)})
                    return
                if self.path == "/ops/ab-test":
                    self._send(HTTPStatus.OK, {"ok": True, "action": "ab-test", "result": run_ab_test(db)})
                    return
                if self.path == "/ops/security-check":
                    self._send(HTTPStatus.OK, {"ok": True, "action": "security-check", "result": security_check()})
                    return
                self._send(HTTPStatus.NOT_FOUND, {"error": "not found", "path": self.path})
