...
<html><head><meta charset='utf-8'><title>{APP_NAME} menu v{SEMVER}</title>
<style>
...
<header><strong>{APP_NAME} Menu v{SEMVER}</strong></header>
<div class='grid'>
  <div class='card'><a href='/dashboard'>Dashboard</a><br><small>Visual demo app with map + camera + lamp simulation.</small></div>
  <div class='card'><a href='/datasets'>Datasets</a><br><small>Visual card list of sources, license and usage.</small></div>
...
def datasets_html() -> str:
    ...
    for row in DatasetRegistry.datasets():
        cards.append(
            f"<div class='card'><h3>{row['name']}</h3><p><b>Scope:</b> {row['scope']}</p><p><b>License:</b> {row['license']}</p><p><b>Usage:</b> {row['usage']}</p><p><a href='{row['url']}' target='_blank'>source</a></p></div>"
        )
    joined = "".join(cards)
    return f"""<!doctype html><html><head><meta charset='utf-8'><title>{APP_NAME} datasets</title>
...
</head><body><header><a href='/menu' style='color:#8ec1ff'>Menu</a> · Datasets</header><div class='grid'>{joined}</div></body></html>"""
...
video{{width:100%;height:280px;border:0;background:#000}}
#mapCanvas{{width:100%;height:320px;background:#0d1119;border:1px solid #2f3a55}}
...
<nav><a href='/menu'>Menu</a><a href='/datasets'>Datasets</a><a href='/architecture'>Architecture</a><a href='/health'>Health</a></nav></header>
...
<div class='card'><h3>Map + Traffic Lamp Visual</h3><canvas id='mapCanvas' width='640' height='320'></canvas><pre id='vision'>vision: pending</pre></div>
<div class='card'><h3>Random dataset demo</h3><button id='runDemo'>Run random sample</button><pre id='sampleInfo'>No sample selected</pre></div>
<div class='card'><h3>Agent output</h3><pre id='demoEvents'>No event yet</pre></div>
</div></div>
<script>
function drawScenario(sample){{
  const c=document.getElementById('mapCanvas'); const x=c.getContext('2d');
  x.fillStyle='#102030'; x.fillRect(0,0,c.width,c.height);
  x.fillStyle='#2f3f55'; x.fillRect(0,140,c.width,70);
  x.strokeStyle='#d7d26a'; x.lineWidth=2; x.setLineDash([10,10]); x.beginPath(); x.moveTo(0,175); x.lineTo(c.width,175); x.stroke(); x.setLineDash([]);
  const lampX=530, lampY=90, r=18;
  x.fillStyle='#222'; x.fillRect(lampX-20,lampY-60,40,110);
  const state = sample.frame.candidates[0].state;
  const color = state==='red' ? '#e53935' : (state==='green' ? '#43a047' : '#fdd835');
  x.fillStyle=color; x.beginPath(); x.arc(lampX,lampY,r,0,Math.PI*2); x.fill();
  x.fillStyle='#fafafa'; x.fillText('route: '+sample.frame.route_id, 20, 30);
  return {{x:lampX,y:lampY}};
}}

function detectLampColorAt(pos){{
  const c=document.getElementById('mapCanvas'); const x=c.getContext('2d');
  const pix=x.getImageData(pos.x,pos.y,1,1).data;
  const [r,g,b]=pix;
  if(r>g && r>b) return 'red';
  if(g>r && g>b) return 'green';
  return 'yellow_or_unknown';
}}
...
(async()=>{{
  ...
  document.getElementById('runDemo').onclick=async()=>{{
    const r=await fetch('/demo/random',{{method:'POST'}}); const d=await r.json();
    const pos=drawScenario(d.sample);
    const visualState=detectLampColorAt(pos);
    document.getElementById('vision').textContent='vision detected: '+visualState;
        ...
        class Handler(BaseHTTPRequestHandler):
            ...
            def do_GET(self) -> None:  # noqa
                ...
                if self.path == "/architecture":
                    self._send(HTTPStatus.OK, {"mobile_plan": ["iOS/Android native camera feed", "on-device model inference (CoreML/TFLite/ONNX)", "AAOS/CarPlay projection adapter"], "current_demo": "Visual map+lmap simulation + rule-based AI agent"})
                    return
            ...
            def do_POST(self) -> None:  # noqa
                if self.path == "/demo/random":
                    ...
                    rows = [json.loads(row) for row in bs.iter_demo_frames()]
                    if not rows:
                        self._send(HTTPStatus.BAD_REQUEST, {"error": "no demo frames"})
                        return
                    frame_obj = random.choice(rows)
                    sample = {"frame": frame_obj, "source": "demo_sample_frames"}
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="Traffic AI Assist v0.7.0")
