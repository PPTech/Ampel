...
class TrafficLightDetector:
    ...
    def __init__(
        self, model_path: str, threshold: float = 0.35, label_map: dict[int, str] | None = None
    ) -> None:
    ...
    def detect(self, image: np.ndarray) -> list[DetectionResult]:
        ...
        if hasattr(np, "array"):
            ...
            for i, score in enumerate(scores[0]):
                ...
                crop = image[
                    int(y1 * image.shape[0]) : int(y2 * image.shape[0]),
    ...
    def evaluate_frame(
        self, detections: list[DetectionResult], at_intersection: bool
    ) -> TemporalAlertResult:
        now = time.monotonic()
    ...
    @staticmethod
    def _select_traffic_light(detections: list[DetectionResult]) -> DetectionResult | None:
        candidates = [
            d
            for d in detections
